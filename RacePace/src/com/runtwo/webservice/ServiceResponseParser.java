...
public class ServiceResponseParser {
	...
	public static String registerResponseParser(Globals global,JSONArray data)
	{
		...
		try {
			...
			String profile_completion = job.getString(GlobalConstants.PROFILE_COMPLETION);
			String total_completion = job.getString(GlobalConstants.TOTAL_PROFILE_COMPLETION);
			//adding to hashmap
	...
	public static String followerFeedParser(Globals global,JSONArray data)
	{
		...
		try {
			...
			for(int i=0;i<size;i++){
				...
				String comments_ct = job.getString(GlobalConstants.FEED_COMMENTS_COUNT);
				String feed_desc = job.getString(GlobalConstants.FEED_DESC);
				String username = job.getString(GlobalConstants.FEED_USERNAME);
				String profile_pic = job.getString(GlobalConstants.FEED_PROFILE_IMG);
	...
	//Follower feed Parser
	public static String followersParser(Globals global,JSONArray data,String type)
	{
		String result = "true";
		try {
			ArrayList<HashMap<String,String>> list = new ArrayList<HashMap<String,String>>();
			...
			for(int i=0;i<size;i++){
				...
				String rp_frnd_id = job.getString(GlobalConstants.FOLLOWERS_RP_FRIEND_ID);
				String friend_id = job.getString(GlobalConstants.FOLLOWERS_FRIEND_ID);
				String catid = job.getString(GlobalConstants.FOLLOWERS_CAT_ID);
				String user_id = job.getString(GlobalConstants.FOLLOWERS_USER_ID);
				String username = job.getString(GlobalConstants.FOLLOWERS_USERNAME);
				String profile_img = job.getString(GlobalConstants.FOLLOWERS_PROFILE_IMG);
				String total_dist = job.getString(GlobalConstants.FOLLOWERS_TOTAL_DIST);
	...
	public static String likeCommentParser(Globals global, JSONArray data,String type) {
		String result = "true";
		try {
			ArrayList<HashMap<String,String>> list = new ArrayList<HashMap<String,String>>();
			int size = data.length();
			...
			for(int i=0;i<size;i++){ 
				...
				String commentdesc = job.getString(GlobalConstants.LIKE_COMMENT_DESC);
				String username = job.getString(GlobalConstants.USERNAME);
				String profileimg = job.getString(GlobalConstants.PROFILE_IMG);
				String rpcommentid = job.getString(GlobalConstants.RP_COMMENT_ID);
	...
	public static String searchFriendsParser(Globals global,JSONArray data,String type) {
		String result = "true";
		try {
			ArrayList<HashMap<String,String>> list = new ArrayList<HashMap<String,String>>();
			int size = data.length();
			...
			for(int i=0;i<size;i++){
				...
				String coins = job.getString(GlobalConstants.COINS);
				String total_distance = job.getString(GlobalConstants.TOTAL_DISTANCE);
				String group_count = job.getString(GlobalConstants.GP_COUNT);
	...
	public static String dashboardContentParser(Globals global,JSONArray data) {
		...
		try {
			...
			String username = job.getString(GlobalConstants.USERNAME);
			String gender = job.getString(GlobalConstants.GENDER);
			String friends_ct = job.getString(GlobalConstants.DASHBOARD_FRIENDS_CT);
			String watchct = job.getString(GlobalConstants.DASHBOARD_WATCHLIST_CT);
			String dist_by_age = job.getString(GlobalConstants.DASHBOARD_DISTANCEBY_AGE);
			String compare_all = job.getString(GlobalConstants.DASHBOARD_COMPARE_ALL);
